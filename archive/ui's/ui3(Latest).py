...
# CSS personalizado para melhorar a aparência da validação
st.markdown("""
<style>
...
/* Estilo para o campo com label e valor */
.field-container {
    margin-bottom: 15px;
}
...
def create_validated_field(label, field_key, current_value=""):
    """Cria um campo de input com nome do campo e status de validação"""
    ...
    # Container do campo com label claro
    st.markdown(f"""
    ...
    if validation_result and validation_result.message:
        st.markdown(f'<small class="{css_class}">{icon} {validation_result.message}</small>', 
                   unsafe_allow_html=True)
...
def process_form_submission():
    ...
    form_keys_mapping = {
        ...
        "total_amount": "TotalDocumentAmount",
        "net_amount": "NetDocumentAmount",
        "vat_amount": "VATAmount",
        "vendor_name": "VendorName",
        "vendor_tax_id": "VendorTaxID",
        "vendor_country": "VendorCountryCode",
        "customer_name": "CustomerName",
        "customer_tax_id": "CustomerTaxID",
        "customer_country": "CustomerCountryCode"
    }
    
    for form_key, json_key in form_keys_mapping.items():
        st.session_state.invoice_data[json_key] = st.session_state.get(form_key, "")
